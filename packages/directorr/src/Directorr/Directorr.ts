...
export class Directorr implements DirectorrInterface {
  ...
  private initStore<I = any>(StoreConstructor: DirectorrStoreClassConstructor<I>): I {
    ...
    if (!hasOwnProperty(store, INJECTED_DIRECTORR_FIELD_NAME))
      defineProperty(store, INJECTED_DIRECTORR_FIELD_NAME, createValueDescriptor(this))
    ...
    // call init action
    if (hasOwnProperty(StoreConstructor, INJECTED_STORES_FIELD_NAME)) {
      const injectedConstructors: DirectorrStore[] = (StoreConstructor as any)[
        INJECTED_STORES_FIELD_NAME
      ]
      const storeNames = injectedConstructors.map(s => getStoreName(s))

      if (checkStoresState(this.stores, isStoreReady, storeNames)) {
        this.dispatchType(DIRECTORR_INIT_STORE_ACTION, { store })
      } else {
        void this.waitStoresState(injectedConstructors).then(() => {
          this.dispatchType(DIRECTORR_INIT_STORE_ACTION, { store })
        })
      }
    } else {
  ...
  private destroyStore(
    ...
    if (store) {
      ...
      if (hasOwnProperty(StoreConstructor, INJECTED_STORES_FIELD_NAME)) {
        const injectedConstructors: any[] = (StoreConstructor as any)[INJECTED_STORES_FIELD_NAME]

        try {
          for (const constructor of injectedConstructors) {
            this.destroyStore(constructor, StoreConstructor)
          }
        } catch (error) {
          if (error instanceof RangeError) {
            throw new TypeError(haveCycleInjectedStore(MODULE_NAME))
  ...
  waitAllStoresState(isStoreState: CheckStoreState = isStoreReady) {
    return createPromiseCancelable<any>((res, _, whenCancel) => {
