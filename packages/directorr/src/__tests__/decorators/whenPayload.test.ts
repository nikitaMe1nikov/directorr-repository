import whenPayload, { initializer, payloadChecker, MODULE_NAME } from '../../decorators/whenPayload'
import {
...
describe('whenPayload', () => {
  it('call payloadChecker with payload = undefined', () => {
    ...
    expect(payloadChecker(payload, valueFunc, [checkerFalse, converterToNewObj])).toBe(convertedObj)
    expect(checkerFalse).toBeCalledTimes(0)
